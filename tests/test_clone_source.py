...
class TestCloneSourceOverride(unittest.TestCase):
    def _constants_with_env(self, env_value=None):
        env_assignment = (
    ...
    def test_override_points_clone_at_custom_source_single_mirror(self):
        custom = "https://git.internal/NyxNiri.git"
        clone_url, repo_url, registry = self._constants_with_env(custom)
        self.assertEqual(clone_url, custom)
        self.assertEqual(registry, f"[('Custom', '{custom}')]")

    def test_display_repo_url_stays_official_even_when_overridden(self):
        _, repo_url, _ = self._constants_with_env("https://git.internal/NyxNiri.git")
        self.assertTrue(repo_url.endswith("ech678/NyxNiri.git"))

    def test_default_without_env_unchanged(self):
        clone_url, repo_url, registry = self._constants_with_env(None)
