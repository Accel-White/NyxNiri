...
def _edit_flat_config(content, changes):
    """Edit Fcitx add-on options and migrate the old invalid section header."""
    _parse_ini(content)
    lines = content.splitlines(keepends=True)
    active_section = None
    root = []
    ...
    for line in lines:
        ...
        if match:
            ...
            continue
        if active_section == "ClassicUI" or not sections:
            root.append(line)
        else:
...
def fcitx_set_theme_conf() -> None:
    """Update Theme and DarkTheme in the root-level classicui.conf format."""
    ...
    updated = _edit_flat_config(content, {"Theme": FCITX_THEME, "DarkTheme": FCITX_THEME})
    _write_config(classicui, updated)
...
def fcitx_restart() -> None:
    """Reload ClassicUI in place, or request a cold start during installation."""
    if not fcitx5_installed():
        return
    result = timed_run(
        FCITX_CLASSICUI_RELOAD,
...
def _restore_settings(path, state_file, section, owned, *, flat=False):
    ...
    if path.is_file():
        ...
        updated = _edit_flat_config(content, changes) if flat else _edit_ini(content, section, changes)
...
def fcitx_uninstall() -> bool:
    ...
    _restore_settings(
        ...
        state_file,
        "ClassicUI",
        {"Theme": FCITX_THEME, "DarkTheme": FCITX_THEME},
        flat=True,
    )
