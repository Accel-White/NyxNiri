...
def _update_flat_config(file_path: Path, key: str, val: str) -> None:
    """Update a top-level Fcitx addon option and migrate our old invalid header."""
    file_path.parent.mkdir(parents=True, exist_ok=True)
    content = ""
    if file_path.is_file():
        content = file_path.read_text(encoding="utf-8", errors="replace")

    # classicui.conf is a flat addon config. Older NyxNiri versions mistakenly
    # added this section, causing Fcitx to ignore Theme and DarkTheme entirely.
...
def fcitx_restart() -> None:
    """Reload the running ClassicUI theme, or start fcitx5 when absent."""
    if fcitx5_installed():
        res = timed_run([
            "busctl", "--user", "call",
            ...
            "s", "classicui",
        ], 5, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
...
def fcitx_configure_trigger_key() -> bool:
    """Set Ctrl+Space on first setup without replacing an existing choice."""
    env = get_env()
    ...
    if niri_config.is_file():
        try:
            ...
            if re.search(r"\bCtrl\+space\b", content, re.IGNORECASE):
    ...
    if "[Hotkey/TriggerKeys]" in content:
        ...
        new_content = content.rstrip() + sep + "[Hotkey/TriggerKeys]\n0=Ctrl+space\n"

    try:
        config_path.write_text(new_content, encoding="utf-8")
        log_msg("INFO", "已自动配置 fcitx5 触发键为 Ctrl+space")
        return True
    except OSError as e:
