...
**Noctalia Greeter：** 和 Noctalia 主题一致的 greetd 登录界面。`nyxniri greeter install` 装 `greetd` + `noctalia-greeter`（AUR），备份现有配置并写入 Polkit 规则；全部准备就绪后才将已启用的显示管理器切到 greetd，验证失败会恢复原显示管理器；不会停止当前图形会话，重启后生效
